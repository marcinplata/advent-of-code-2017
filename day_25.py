...
def read_file(path):
    ...
    with open(path) as file:
        begin_state = file.readline()[-3]
        steps = int("".join(c for c in file.readline() if c.isdigit()))
        file.readline()
        while True:
            state = file.readline()[-3]
            value = int(file.readline()[-3])
            write = int(file.readline()[-3])
            direction = file.readline().split(" ")[-1][:-2]
            next_state = file.readline()[-3]
            states[(state, value)] = (write, direction, next_state)
            value = int(file.readline()[-3])
            write = int(file.readline()[-3])
            direction = file.readline().split(" ")[-1][:-2]
            next_state_line = file.readline()
            next_state = next_state_line[-3] if next_state_line[-1] == "\n" else next_state_line[-2]
            states[(state, value)] = (write, direction, next_state)
...
def run_turing_machine(state, steps, states):
    tape = defaultdict(lambda: 0)
    position = 0
    for i in range(steps):
        tape[position], direction, state = states[(state, tape[position])]
        position = position+1 if direction == 'right' else position-1
    return sum(1 for k in tape.keys() if tape[k] == 1)


state, steps, states = read_file("tests/day_25_test")
print(run_turing_machine(state, steps, states))
