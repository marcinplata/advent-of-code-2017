...
def read_file(path):
    ...
    with open(path) as file:
        for i, line in enumerate(file):
            ...
            for j, s in enumerate(list(line)):
                if s == '#':
                    hashes.add((j, i))
    return hashes, (i//2, j//2)


def rotate(dir, ang):
    ang = math.radians(ang)
    return int(dir[0] * math.cos(ang) - dir[1] * math.sin(ang)), int(dir[0] * math.sin(ang) + dir[1] * math.cos(ang))


def go(position, hashes):
    direction = (0, -1)
    count = 0
    for i in range(10000):
        if position in hashes:
            # turn right (rotated board)
            direction = rotate(direction, 90)
            hashes.remove(position)
        else:
            # turn left (rotated board)
            direction = rotate(direction, 270)
            hashes.add(position)
            count += 1
...
# not required for passing the assignment
def print_pos(hashes):
    for i in range(-10, 5):
        line = ""
...
hashes, middle = read_file("tests/day_22_test")
print(go(middle, hashes))
...
def go_with_more_states(position, hashes):
    hashes = {v: '#' for v in hashes}
    direction = (0, -1)
    count = 0
    for i in range(10000000):
        if position in hashes.keys():
            if hashes[position] == '#':
                # turn right (rotated board)
                direction = rotate(direction, 90)
                hashes[position] = 'F'
            elif hashes[position] == 'W':
                hashes[position] = '#'
                count += 1
            elif hashes[position] == 'F':
...
hashes, middle = read_file("tests/day_22_test")
print(go_with_more_states(middle, hashes))
