...
that (with range 2), a third layer which begins at depth 4 (with range 4), and a fourth layer which begins at depth 6
...
do not get caught.) The severity of the whole trip is the sum of these values. In the example above,
...
def read_file(path):
    ...
    with open(path) as file:
        for line in file:
            ...
            firewall_seed.append([int(x) for x in line.replace(" ", "").split(":")])
    return firewall_seed
...
def go_throw_firewall(struct):
    caught = []
    for i in range(max(struct.keys())+1):
        if i in struct and i % struct[i] == 0:
            caught.append(i*(struct[i]+2)//2)
    return sum(caught)


firewall_seed = read_file("tests/day_13_test")
firewall = create_firewall(firewall_seed)
print(go_throw_firewall(firewall))
...
def find_delay_with_zero_caught(struct):
    delay = 3800000
    caught = -1
    while caught != 0:
        delay += 1
        caught = go_throw_firewall_with_delay(struct, delay)
    return delay


print(find_delay_with_zero_caught(firewall))
