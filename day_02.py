...
rows = read_file("tests/day_02_test")
print(checksum(rows))
...
def checksum(rows):
    sum = 0
    for row in rows:
        pairs = [(x, y) for i, x in enumerate(row) for y in row[i+1:]]
        pairs = list(map(lambda x: (x[1], x[0]) if x[0] < x[1] else x, pairs))
        pairs = list(filter(lambda x: x[0] % x[1] == 0, pairs))
        x, y = pairs[0]
        sum = sum + x//y
    return sum
