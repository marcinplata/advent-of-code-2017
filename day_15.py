...
# print(generate_numbers(679, 771))
...
Generator A looks for values that are multiples of 4. Generator B looks for values that are multiples of 8. Each 
generator functions completely independently: they both go through values entirely on their own, only occasionally 
handing an acceptable value to the judge, and otherwise working through the same sequence of values as before until 
they find one. 
...
def generate_numbers_with_multiples(a, b):
    matched = 0
...
print(generate_numbers_with_multiples(679, 771))
