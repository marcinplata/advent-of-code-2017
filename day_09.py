...
def process_stream(stream):
    ...
    i = 0
    while i < len(stream):
        if stream[i] == "{" and not garbage_open:
            group.append(group[-1] + 1)
        elif stream[i] == "}" and not garbage_open:
            done.append(group.pop())
        elif stream[i] == "<":
            garbage_open = True
        elif stream[i] == ">":
            garbage_open = False
...
stream = read_file("tests/day_09_test")
...
def process_garbage_size(stream):
    garbage_size = 0
    garbage_open = False
    i = 0
    while i < len(stream):
...
print(process_garbage_size(stream))
