...
def run_instructions(instructions):
    ...
    while pos < len(instructions):
        ...
        if i == 'set':
            ...
            registers[r] = registers[r] % registers[v]
        elif i == "jgz" and registers[r] > 0:
            pos = pos + registers[v] - 1
        elif i == "snd":
            send.append(registers[r])
        elif i == "rcv" and registers[r] > 0:
            return send[-1]
        pos += 1
    return -1


insts = read_file("tests/day_18_test")
...
the programs don't even necessarily run at the same speed. To coordinate, they use the send (snd) and receive (rcv) 
instructions: 
...
each program receives a value (both 1) and stores it in a, receives another value (both 2) and stores it in b, 
and then each receives the program ID of the other program (program 0 receives 1; program 1 receives 0) and stores it 
in c. Each program now sees a different value in its own copy of register c. 
...
program 0 might have sent all three values and then stopped at the first rcv before program 1 executed even its first 
instruction. 
...
def run_instructions_with_registers(instructions, registers, pos, from_other):
    for_other = []
    while pos < len(instructions):
        i, r, v = instructions[pos]
        if i == 'set':
            ...
            for_other.append(registers[r])
        elif i == "rcv":
            if len(from_other) > 0:
                registers[r] = from_other[0]
                from_other.pop(0)
            else:
                return registers, pos, for_other
        pos += 1
    return -1
...
def run_program(instructions):
    ...
    r = {n: 0 for n in list(instructions[:, 1]) + list(instructions[:, 2]) if not n.lstrip("-").isdigit()}
    registers_0 = {**n, **r}
    r = {n: 1 for n in list(instructions[:, 1]) + list(instructions[:, 2]) if not n.lstrip("-").isdigit()}
    registers_1 = {**n, **r}
    send_0, send_1 = [], []
    pos_0, pos_1 = 0, 0
    send_0_count = 0
    while True:
        registers_0, pos_0, send_1 = run_instructions_with_registers(instructions, registers_0, pos_0, send_0)
        registers_1, pos_1, send_0 = run_instructions_with_registers(instructions, registers_1, pos_1, send_1)
        send_0_count += len(send_0)
        if not send_0 and not send_1:
            return send_0_count


print(run_program(insts))
