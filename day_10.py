...
to reverse the order of the marks within it. After doing this many times, the order of the marks is used to build the
resulting hash.
...
def make_hash(moves):
    numbers, n = list(range(0, 256)), 256
    actual_pos = 0
    for i, m in enumerate(moves):
        for j, k in enumerate(range(actual_pos, actual_pos+(m//2))):
            numbers[k % n], numbers[(k+(m-2*j-1)) % n] = numbers[(k+(m-2*j-1)) % n], numbers[k % n]
        actual_pos += (m + i) % n
    return numbers[0] * numbers[1]


moves = read_file("tests/day_10_test")
print(make_hash(moves))
...
31, 73, 47, 23. For example, if you are given 1,2,3, your final sequence of lengths should be 49,44,50,44,51,17,31,
73,47,23 (the ASCII codes from the input string combined with the standard length suffix values). 

Second, instead of merely running one round like you did above, run a total of 64 rounds, using the same length 
sequence in each round. The current position and skip size should be preserved between rounds. For example, 
if the previous example was your first round, you would start your second round with the same length sequence (3, 4, 
1, 5, 17, 31, 73, 47, 23, now assuming they came from ASCII codes and include the suffix), but start with the 
...
Perform this operation on each of the 
sixteen blocks of sixteen numbers in your sparse hash to determine the sixteen numbers in your dense hash. 

Finally, the standard way to represent a Knot Hash is as a single hexadecimal string; the final output is the dense 
hash in hexadecimal notation. Because each number in your dense hash will be between 0 and 255 (inclusive), 
always represent each number as two hexadecimal digits (including a leading zero as necessary). So, if your first 
three numbers are 64, 7, 255, they correspond to the hexadecimal numbers 40, 07, ff, and so the first six characters 
of the hash would be 4007ff. Because every Knot Hash is sixteen such numbers, the hexadecimal representation is 
...
def read_file_with_raw_string(path):
    with open(path) as file:
        line = file.read()
...
moves = read_file_with_raw_string("tests/day_10_test")
print(run_hashing_loop(moves))
