...
def read_file(path):
    with open(path) as file:
        memory = file.read()
    memory = memory[:-1] if memory[-1] == '\n' else memory
    return list(map(lambda x: int(x), memory.split("\t")))


def reallocation(memory):
    n = len(memory)
    reverse = list(range(n-1, -1, -1))
    iter_memory = list(zip(memory, reverse))

    step = 0
    conf = set()
    while tuple(iter_memory) not in conf:
        conf.add(tuple(iter_memory))
        step += 1
        max_val, max_i = max(iter_memory)
        i = n - max_i - 1
        #print(max_val, i)
        quotient = max_val // n
        remainder = max_val % n
        iter_memory[i] = (0, iter_memory[i][1])
        iter_memory = [(m+quotient, i) for m, i in iter_memory]
        for ii in range(i+1, i+1+remainder):
            ii = ii % n
            iter_memory[ii] = (iter_memory[ii][0]+1, iter_memory[ii][1])
        # print(iter_memory)
    return step


memory = read_file("tests/day_06_test")
print(reallocation(memory))
...
def reallocation_with_distance(memory):
    n = len(memory)
    reverse = list(range(n-1, -1, -1))
    iter_memory = list(zip(memory, reverse))
    ...
    step = 0
    conf = {}
    while tuple(iter_memory) not in conf.keys():
        conf[tuple(iter_memory)] = step
        step += 1
        max_val, max_i = max(iter_memory)
        i = n - max_i - 1
        # print(max_val, i)
        ...
        for ii in range(i+1, i+1+remainder):
            ...
            iter_memory[ii] = (iter_memory[ii][0]+1, iter_memory[ii][1])
        # print(iter_memory)
    return step - conf[tuple(iter_memory)]


memory = read_file("tests/day_06_test")
print(reallocation_with_distance(memory))